...
- **Per-Stage Model Config**: Each enrichment stage can use a different Gemini model via `STAGE_MODELS` in `src/lib/ai/config.ts`. All stages default to `GEMINI_MODEL` from `constants.ts`. Change individual stages without affecting the rest of the pipeline. Runtime config stored in `src/lib/ai/runtime-config.ts` and persisted to `/tmp/ai-stage-config.json`.
- **Admin AI Config Page**: `/admin/ai-config` provides a UI for configuring per-stage enrichment parameters: model selection, search grounding toggle, thinking level, temperature, dynamic threshold, and max retries. Changes are saved to runtime config and take effect on the next enrichment run. API at `/api/admin/ai-config` (GET/PUT). Model-aware UI adapts options based on selected model (e.g., thinking level only for Gemini 2.5+/3, dynamic threshold only for non-Gemini-3 with search grounding enabled).
