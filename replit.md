...
- **Parcel Resolution**: NEVER use spatial proximity matching. Parcel hover/click uses exact + prefix matching against Regrid parcel numbers, with `/api/parcels/resolve` API fallback. Always resolves to parent property. Every Regrid parcel has a ll_uuid and parcelnumb that should match our Snowflake-ingested data. Tooltip, highlighting, and click must always resolve to the parent/primary property name - never show individual owner names from Regrid tile data for properties we have in our database.
