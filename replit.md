...
- **Gemini Timeout Fix**: `callGeminiWithTimeout` timeout only covers actual API call time (not queue wait). Internal error messages stripped from user-facing summaries via `stripInternalMessages()` regex fallback. Uses SDK-native `httpOptions.timeout` at 600s via Vertex AI (up from 280s). Runtime patch applied to `@google/genai` SDK to inject `undici` Agent with matching `headersTimeout`/`bodyTimeout`, bypassing Node.js's hardcoded 300s fetch header timeout (SDK bug googleapis/js-genai#1277). Patch applied on first `getGeminiClient()` call to both `.mjs` and `.cjs` SDK builds. Smart retry with extended backoff for DEADLINE_EXCEEDED/504 errors.
