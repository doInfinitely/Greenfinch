      ...
      {contacts.length > 0 ? (
        <div className="space-y-3">
          {sortContactsByRelevance(contacts).map((contact, i) => {
            ...
            const rowClasses = `block p-4 rounded-lg transition-colors ${isFormer ? 'bg-gray-100 opacity-70' : 'bg-gray-50'} ${contact.id ? 'cursor-pointer hover:bg-gray-100' : ''}`;
